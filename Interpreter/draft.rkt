    ...
    (lambda (exp)
        (cases expression exp
            ...
            [case-exp (key tests results)
				(if (null? (cdr tests))
					(if-one-exp (case-or-expression key (1st tests))
...
(define eval-exp
    ...
    (cases expression exp
			...
			(apply-env env
                        ...
                        (lambda ()
                            (apply-env init-env
                                       id
                                       (lambda (x) x)
                                       (lambda () (error 'apply-env ; procedure to call if id not in env
        ...
        [while-exp (condition exp)
            (if (eval-exp condition env)
                (begin (eval-bodies exp env) (eval-exp (while-exp condition exp) env))
                #f)]
        ; [let-exp (syms exps bodies)
        ;     (let ([extended-env
        ;         (extend-env syms
